...
- Notification 서비스는 사용자의 주의를 환기시키고, 알림 받은 정보를 통해 유의미한 행위를 유도할 수 있는 서비스입니다.
...
- 로그인 기능 구현에 있어 Google OAuth 사용
    ...
    - ( 시큐리티 관련 이슈 하나 적어주면 좋을 듯함 )
- Validation을 사용하여 키워드 입력에 대한 검증 로직 추가
    - ( 밸리데이션 관련 한 이슈 하나 적어주면 좋을 듯함 )
- 읽기 성능 향상을 목적으로 캐시적용
...
- `처리 지연의 방지`를 위한 `EDP`와 `비동기 처리`
    - 알림을 생성하고 발송하는 모든 단계를 한 흐름으로 처리할 경우 처리지연이 발생함 ( DB 조회 및 STMP로 메일 발송 )
